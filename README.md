...
synchronization, or unmanaged-skill adoption is confirmed. An adoptable
inventory entry requires a separately chosen exact qualified library ID and an
exact content match; adoption records state only and never replaces target
files. Viewing an unmanaged skill never adopts, overwrites, publishes, or
deletes it. Use regular commands for pipes, CI, `--json`, `--no-input`, `--yes`,
